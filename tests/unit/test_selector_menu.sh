...
assert_contains "$selector" 'echo "13) Configure Network Policy"'
assert_contains "$selector" 'echo "14) Print Effective Config"'
...
assert_contains "$selector" '13) run_sector_safe "$SECTOR_DIR/32-network-apply.sh" ;;'
assert_contains "$selector" '14) run_sector_safe "$SECTOR_DIR/92-print-effective-config.sh" ;;'
